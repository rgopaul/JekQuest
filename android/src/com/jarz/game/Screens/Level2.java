...
import com.jarz.game.Sprites.Player.Player_Level2;
...
/*
* The Second Level of the Game (NOTE THIS LEVEL IS INCOMPLETE AND IS REPLACED BY LEVEL 3):
...
public class Level2 implements Screen
{
    ...
    * */
    public Level2(Platformer game, HUD hud, int score)
    {
        atlas = new TextureAtlas("Jekplz2.pack"); //Grab Main Character Sprite Information
        ...
        HUD = hud; //Instantiate The Score Display of the Game
        HUD.setTime(20);
        HUD.setScore(score);

        //Load Map and Assign a Camera / Renderer to the Game
        mapLoader = new TmxMapLoader();
        map = mapLoader.load("Stages/cutman.tmx");
        mapProp = map.getProperties();
        renderer = new OrthogonalTiledMapRenderer(map, 1 / Platformer.PPM);
        gamecam.position.set(gamePort.getWorldWidth() / 2, gamePort.getWorldHeight() / 2, 0);

        world = new World(new Vector2(0,-7.8f), true); //First Parameter Sets Gravity
    ...
    public void handleInput(float dt)
    {
        if(player.currentState != Player_Level2.State.DEAD)
        {
            ...
            {
                latch = true;
                player.b2body.setGravityScale(Default_Gravity);
                //keyboard controls
                ...
                if (controller.isRightPressed() && player.b2body.getLinearVelocity().x <= 1.0) {
                    Gdx.app.log("Touched", "RIGHT");
                    player.b2body.applyLinearImpulse(new Vector2(0.1f, 0), player.b2body.getWorldCenter(), true);
                }

                if (controller.isLeftPressed() && player.b2body.getLinearVelocity().x >= -1.0) {
            ...
            {
                /*
                * These statements are only executed when the player is in a "Climbing" state.
                * Meaning they are currently interacting with a Ladder within the Level.
                * This is NOT fully implemented and is left here for demonstration purposes.
                * */
                if(latch)
                {
                    player.b2body.setLinearVelocity(0, 0.0f);
                    player.b2body.setGravityScale(0.0f);
                    latch = false;
                }

                if (Gdx.input.isKeyPressed(Input.Keys.UP) && player.b2body.getLinearVelocity().x < 0.02f)
                {
                    player.b2body.applyLinearImpulse(new Vector2(0, 0.02f), player.b2body.getWorldCenter(), true);
                    player.b2body.setGravityScale(0.0f);
                }
                ...
                else if (controller.isUpPressed() && player.b2body.getLinearVelocity().x < 0.02f)
                {
                    player.b2body.applyLinearImpulse(new Vector2(0, 0.02f), player.b2body.getWorldCenter(), true);
                    player.b2body.setGravityScale(0.0f);
                }
